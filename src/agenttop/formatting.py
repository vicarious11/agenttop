...
def format_budget_message(budget_info: BudgetInfo) -> tuple[str, str]:
    ...
    Returns:
        (message, color) tuple for click.echo() with fg=color.
    """
    if budget_info.status == BudgetStatus.ALERT:
        return (
            f"⚠️  OVER BUDGET: ${budget_info.total_cost:.2f} "
            f"({budget_info.ratio:.0%} of ${budget_info.budget:.2f} limit)",
            "red"
        )
    elif budget_info.status == BudgetStatus.WARNING:
        return (
            f"⚠️  ${budget_info.total_cost:.2f} "
            f"({budget_info.ratio:.0%} of ${budget_info.budget:.2f} daily budget)",
            "yellow"
        )
    else:
        return (
            f"${budget_info.total_cost:.2f} of ${budget_info.budget:.2f} daily budget",
            "green"
        )
